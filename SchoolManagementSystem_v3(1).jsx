...
function AdminDashboard({onBack,dark,onToggleTheme,lang,onToggleLang,creds,onCredsChange,gradePeriods,onPeriodsChange,studentGrades,onStudentGradesChange}){
  ...
  useEffect(()=>{
    async function load(){
      const[dS,dT]=await Promise.all([sb.get("students","?order=created_at.asc"),sb.get("teachers","?order=created_at.asc")]);
  ...
  const addCred=()=>{if(!nc.email||!nc.password||!nc.name)return;onCredsChange({...creds,[nc.email]:{password:nc.password,name:nc.name,role:nc.role,metadata:{phone:nc.phone}}});setNc({email:"",password:"",name:"",role:"teacher",phone:""});setModal(null);setToast(t.credAdded);};
